...
Keep the engine's Electron dependency installed: `figma-linux-next` pins
Electron 43.3.0 because Min's Electron 42 runtime can fail Figma's
`/app_auth/redeem` request. The launcher only uses Min's Electron as a last
resort when no engine or system Electron is available.
...
| `src/main/App.ts` | Skip MCP + changelog; start control server; do not steal the `figma://` protocol from the official desktop app. |
...
Copy `engineMode.ts` from this folder into `vendor/figma-linux-next/src/main/` if it is missing after a re-clone, then re-do the other file edits (or keep this clone).
